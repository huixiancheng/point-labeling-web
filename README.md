...
<video controls muted loop playsinline preload="metadata" width="960">
  ...
  <a href="assets/usage_demo.mp4">下载演示视频 / Download demo video</a>
</video>

GitHub 支持 MP4 视频，但 README 中的播放器需要视频已经上传到 GitHub（仓库文件、Release asset 或 GitHub attachment）。当前 `usage_demo.mp4` 保留在本地并按 Release asset 发布策略管理；上传后将 `<source>` 替换为 GitHub 生成的资源 URL，即可在 GitHub 页面播放。H.264 兼容性最好。详情见 [`assets/README.md`](assets/README.md)。
